...
public class PullRequestFragment extends IssueFragmentBase {
   ...
   private void fillStatus(List<Status> statuses) {
    ...
    protected Single<List<TimelineItem>> onCreateDataSingle() {
        ...
        Single<SparseArray<List<ReviewComment>>> reviewCommentsByIdSingle = reviewSingle
                .compose(RxUtils.filter(r -> r.state() == ReviewState.Pending))
                .toObservable()
                .flatMap(reviews -> {
                    List<Observable<Pair<Integer, List<ReviewComment>>>> obsList = new ArrayList<>();
                ...
                .toList()
                .map(list -> {
                    SparseArray<List<ReviewComment>> result = new SparseArray<>();
                    for (Pair<Integer, List<ReviewComment>> pair : list) {
